...
public class WarehouseManager {
	...
	public static void main(String[] args) {
		...
		int intputNum = 0;
		boolean loopFlag;
		while (intputNum < ABKosanArray1.length) {
			int randfive = (int) (Math.random() * 5) + 1;

			loopFlag = false;
			for (int i = 0; i < intputNum; i++) {
				if (ABKosanArray1[i] == randfive) {
					loopFlag = true;
					break;
				}
			}
			if (!loopFlag) {
				ABKosanArray1[intputNum] = randfive;
				intputNum++;
			}
		}
		...
		intputNum = 0;
		while (intputNum < ABKosanArray2.length) {
			int randfive = (int) (Math.random() * 5) + 6;

			loopFlag = false;
			for (int i = 0; i < intputNum; i++) {
				if (ABKosanArray2[i] == randfive) {
					loopFlag = true;
					break;
				}
			}
			if (!loopFlag) {
				ABKosanArray2[intputNum] = randfive;
				intputNum++;
			}
		}
